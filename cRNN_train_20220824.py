...
for i in range(2):
    ...
    #重新训练
    trainer = model_trainer(sub_smile=sub_smiles, model="model_0824", binmols=binmols)
    trainer.init_model(dataset_info)
    trainer.fit()
    trainer.save_model("models/model_0824_{}".format(i))

    #生成分子
    generator = mol_generator(seed_smile=seed_smiles, sub_smile=sub_smiles, model=trainer.model, target_names=target_names)
    generator.sample(10000, [None]*6+[10.0])
    generator.dump("datasets/generator_0824_m_{}.pickle".format(i), "datasets/generator_0824_p_{}.pickle".format(i))

    #更新训练集
    del binmols_list[-1]
    binmols_list.append(get_random_binmols())

    dataset_filename = r"datasets/data_0824_{}.h5".format(i)
...
# 重训
trainer = model_trainer(is_retrain=True, sub_smile=sub_smiles, model="model_0824", binmols=binmols)
trainer.fit()
trainer.save_model("models/model_0824_final")

# 生成
generator = mol_generator(seed_smile=seed_smiles, sub_smile=sub_smiles, model=trainer.model, target_names=target_names)
generator.sample(1000, [None]*6+[10.0])
generator.dump("datasets/generator_0824_m.pickle", "datasets/generator_0824_p.pickle")
