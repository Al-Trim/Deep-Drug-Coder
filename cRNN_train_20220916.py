#换用GASA，尝试使用GC回收内存，采用新的相似度计算策略，阶梯式更改采样条件，去除计算生成分子的性质环节，增加采样量，连续训练4次
import gc, os, sys
import numpy as np
import random
...
for i in range(4):
    ...
    #重新训练
    if i == 0:
        trainer = model_trainer(sub_smile=sub_smiles, model="model_0916", binmols=binmols, descrs=descrs)
    ...
    quit()
    trainer.fit()
    trainer.save_model("models/model_0916_{}".format(i))

    #生成分子
    generator = mol_generator(seed_smile=seed_smiles, sub_smile=sub_smiles, model=trainer.model, target_names=target_names)
    target_gasas = 0.25 + 0.1 * i
    generator.sample(2000//(i+1), [None]*6+[target_gasas])
    generator.dump("datasets/generator_0916_m_{}.pickle".format(i), generator.sani_mols)
    #generator.dump("datasets/generator_0916_p_{}.pickle".format(i), generator.sani_properties)
    ...
    dataset_filename = r"datasets/data_0916_{}.h5".format(i)
