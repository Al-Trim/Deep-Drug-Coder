...
# CHEMBL25
binmols_1, descrs_1 = get_mols_and_descr('datasets/CHEMBL25_TRAIN_MOLS.h5')
binmols_1 = binmols_1[:20000]
descrs_1 = descrs_1[:20000]
# 筛选的相似度0.3-1的分子
...
for i in range(2):
    ...
    gt.fit(model_name='model_1108_{}'.format(i), epochs=100, lr_init=pow(10,-4.5))
    gt.save_model("models/model_1108_{}".format(i))
    ...
    gt.dump("datasets/generator_1108_{}.pickle".format(i), gt.sani_properties)
    gt.plot("results/model_1108_{}.png".format(i), title="model_1108_{}".format(i))
...
# retrain前的预训练
binmols_list =  [binmols_1, binmols_2]
descrs_list = [descrs_1, descrs_2]
binmols = np.concatenate(binmols_list)
descrs = np.vstack(descrs_list)

gt.init_model(x=descrs, y=binmols, dataset_info=dataset_info)
gt.fit(model_name='model_1108_retrain_0', lr=pow(10,-4.5), lr_init=pow(10,-4.5))
gt.save_model("models/model_1108_retrain_0")
...
for i in range(retrain_times):
    ...
    #重训
    gt.retrain("models/model_1108_retrain_{}".format(i), x=descrs, y=binmols, epochs=100, lr=pow(10,-4.5), lr_init=pow(10,-4.5))
    gt.save_model("models/model_1108_retrain_{}".format(i+1))
    ...
    properties = get_descriptors(mols, sub_mol)
    gt.set_properties(properties)
    gt.plot("results/model_1108_retrain_{}.png".format(i), title="model_1108_retrain_{}".format(i))
...
report_data.to_csv('results/report_1108.csv')
