...
class mol_generator:
    ...
    def load_data(self, properties_filename:str = ""):
        ...
        for i in range(len(self.data)):
            self.sani_properties.append(list(self.data.loc[i]))

        print(list(self.data.loc[0]))
        print("Loaded {} mols with properties.".format(len(self.data))) 
...
#generator = mol_generator(seed_smile=seed_smiles, sub_smile=sub_smiles, model='models/model_0921_7', target_names=target_names)

binmols_1_list = []
descrs_1_list = []
for i in range(3):
    generator.load_data_old("datasets/generator_0921_m_{}.pickle".format(i), "datasets/generator_0921_p_{}.pickle".format(i))
    filtered_data = generator.filter_data()
    new_binmols = filtered_data['binmols']
    new_descrs = np.asarray([filtered_data[name] for name in target_names]).T
...
# 合并
binmols_list = binmols_1_list + [binmols_2]
descrs_list = descrs_1_list + [descrs_2]
...
for i in range(4):
    ...
    if i == 0:
        generator = mol_generator(seed_smile=seed_smiles, sub_smile=sub_smiles, target_names=target_names)
        generator.load_data("datasets/generator_0924_0.pickle")
    else:
        #重新训练
        trainer = model_trainer(sub_smile=sub_smiles, model="model_0924", binmols=binmols, descrs=descrs)
        trainer.init_model(dataset_info)
        trainer.fit()
        trainer.save_model("models/model_0924_{}".format(i))
        ...
        generator.dump("datasets/generator_0924_{}.pickle".format(i), generator.data)
