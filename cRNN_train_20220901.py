...
print(descrs_1.shape)
binmols_3 = np.asarray([seed_mol.ToBinary()] * 3)
descrs_3 = np.asarray([get_descriptors(seed_mol, sub_mol)] * 3)
print(descrs_3.shape)
descrs = np.vstack((descrs_1,descrs_3))
print(descrs.shape)
quit()

generator = mol_generator(seed_smile=seed_smiles, sub_smile=sub_smiles, model="models/model_0824_final", target_names=target_names)
generator.sample(4, [None]*6+[1])
generator.dump("datasets/generator_0901_m_test.pickle", generator.sani_mols)
generator.dump("datasets/generator_0901_p_test.pickle", generator.sani_properties)

dataset_filename = r"datasets/data_0901_test.h5"
...
[print(i.shape) for i in descrs_list]
...
for i in range(2):
    ...
    #重新训练
    trainer = model_trainer(sub_smile=sub_smiles, model="model_0901", binmols=binmols, descrs=descrs)
    trainer.init_model(dataset_info)
    trainer.fit()
    trainer.save_model("models/model_0901_{}".format(i))

    #生成分子
    generator = mol_generator(seed_smile=seed_smiles, sub_smile=sub_smiles, model=trainer.model, target_names=target_names)
    generator.sample(10000, [None]*6+[1])
    generator.dump("datasets/generator_0901_m_{}.pickle".format(i), generator.sani_mols)
    generator.dump("datasets/generator_0901_p_{}.pickle".format(i), generator.sani_properties)
    ...
    dataset_filename = r"datasets/data_0901_{}.h5".format(i)
    filtered_binmols_data, filtered_properties_data = generator.filter_data('sub_similarity', 1.0)
    filtered_binmols = filtered_binmols_data['binmols']
    filtered_descrs = np.asarray([filtered_properties_data[name] for name in target_names]).T
...
# 重训
trainer = model_trainer(is_retrain=True, sub_smile=sub_smiles, model="models/model_0901_1", binmols=binmols, descrs=descrs)
trainer.fit()
trainer.save_model("models/model_0901_final")

# 生成
generator = mol_generator(seed_smile=seed_smiles, sub_smile=sub_smiles, model=trainer.model, target_names=target_names)
generator.sample(1000, [None]*6+[1])
generator.dump("datasets/generator_0901_m_final.pickle", generator.sani_mols)
generator.dump("datasets/generator_0901_p_final.pickle", generator.sani_properties)
