...
class mol_generator:
    ...
    def load_data(self, properties_filename:str = ""):
        ...
        # self.data = pd.DataFrame(self.sani_properties, columns=['mol','smiles']+self.target_names) 
        self.sani_properties = [list(self.data.loc[i]) for i in range(len(self.data))]
...
for i in range(4):
    ...
    #generator = mol_generator(seed_smile=seed_smiles, sub_smile=sub_smiles, target_names=target_names)
    generator.load_data("datasets/generator_0924_{}.pickle".format(i))
    generator.dump("datasets/generator_0925_{}.pickle".format(i), generator.sani_properties)
