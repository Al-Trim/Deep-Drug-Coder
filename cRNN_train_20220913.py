#换用GASA，尝试使用GC回收内存，采用新的相似度计算策略，阶梯式更改采样条件，去除计算生成分子的性质环节，连续训练3次
import gc, os, sys
import numpy as np
import random
...
for i in range(3):
    ...
    #重新训练
    trainer = model_trainer(sub_smile=sub_smiles, model="model_0911_{}".format(i), binmols=binmols, descrs=descrs)
