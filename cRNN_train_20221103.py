...
for i in range(2):
    ...
    gt.fit(model_name='model_1103_{}'.format(i), lr_init=pow(10,-4.5))
    gt.save_model("models/model_1103_{}".format(i))
    ...
    gt.dump("datasets/generator_1103_{}.pickle".format(i), gt.sani_properties)
    gt.plot("results/model_1103_{}.png".format(i), title="model_1103_{}".format(i))
...
gt.init_model(x=descrs, y=binmols, dataset_info=dataset_info)
gt.fit(model_name='model_1103_retrain_0', lr=pow(10,-4.5), lr_init=pow(10,-4.5))
gt.save_model("models/model_1103_retrain_0")

# 计算可以retrain的次数
trainset_count = len(filtered_binmols)
retrain_times = trainset_count // 15000
...
for i in range(retrain_times - 1):
    ...
    #更新训练集
    binmols = np.asarray(filtered_binmols[15000*i : 15000*(i+1)])
    descrs = np.asarray(filtered_descrs[15000*i : 15000*(i+1)])

    #重训
    gt.retrain("models/model_1103_retrain_{}".format(i), x=descrs, y=binmols, lr=pow(10,-4.5), lr_init=pow(10,-4.5))
    gt.save_model("models/model_1103_retrain_{}".format(i+1))
    ...
    properties = get_descriptors(mols, sub_mol)
    gt.set_properties(properties)
    gt.plot("results/model_1103_retrain_{}.png".format(i), title="model_1103_retrain_{}".format(i))
...
report_data.to_csv('results/report_1103.csv')
