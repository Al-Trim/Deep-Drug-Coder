#换用GASA描述符，修复内存溢出问题，训练2次+重新训练1次
...
def get_random_binmols():
    ...
    with h5py.File(dataset_filename, "r") as f:
        return np.asarray(f["mols"][rnd_idx:rnd_idx+5000])
...
for i in range(2):
    ...
    #重新训练
    trainer = model_trainer(sub_smile=sub_smiles, model="model_0904", binmols=binmols)
    trainer.init_model(dataset_info)
    trainer.fit()
    trainer.save_model("models/model_0904_{}".format(i))

    #生成分子
    generator = mol_generator(seed_smile=seed_smiles, sub_smile=sub_smiles, model=trainer.model, target_names=target_names)
    generator.sample(2000//(i+1), [None]*6+[1.0])
    generator.dump("datasets/generator_0904_m_{}.pickle".format(i), "datasets/generator_0904_p_{}.pickle".format(i))

    #更新训练集
    del binmols_list[-1]
    binmols_list.append(get_random_binmols())

    dataset_filename = r"datasets/data_0904_{}.h5".format(i)
    generator.filter_data(dataset_filename)
    with h5py.File(dataset_filename, "r") as f:
        binmols_list.insert(0, np.asarray(f["mols"])[:5000])

    binmols = np.concatenate(binmols_list)

# 重训
trainer = model_trainer(is_retrain=True, sub_smile=sub_smiles, model="model_0904_1", binmols=binmols)
trainer.fit()
trainer.save_model("models/model_0904_final")

# 生成
generator = mol_generator(seed_smile=seed_smiles, sub_smile=sub_smiles, model=trainer.model, target_names=target_names)
generator.sample(1000, [None]*6+[1.0])
generator.dump("datasets/generator_0904_m.pickle", "datasets/generator_0904_p.pickle")
