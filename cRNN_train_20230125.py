...
def get_gasas(mols: list=[], path: str='') -> list:
    '''
    计算GASA打分
    '''
    #if default_path[-4:] != "GASA":
    ...
    gasas = gasa.GASA(smiles_list, path)[1]
    #os.chdir(default_path)
    return gasas

def get_descriptors(mol_list, sub_mol, path):
    '''
    计算描述符
    ...
    gasas_list = get_gasas(mol_list, path)
...
step = sys.argv[1]
i = eval(sys.argv[2]) if len(sys.argv) >= 3 else 0
if len(sys.argv) >= 4:
    arg = sys.argv[3]
...
if step == 'main':
    ...
    # 载入
    os.system("python cRNN_train_{}.py load_data >> {}.out".format(MODEL_NAME))
    while True:
        pass
    ...
    filtered_data = pd.concat([filtered_data.reset_index(drop=True), random_data], axis=1)
    filtered_data = filtered_data.sort_values(['random value'])
    
    # reset index是为了防止合并出现nan

    # 计算可以retrain的次数
    trainset_count = len(filtered_data)
    retrain_times = trainset_count // 10000

    # 传递给bash
    dump_data(data=filtered_data, filename='filtered_data_{}'.format(MODEL_NAME))
    ...
    gt.load_model('models/model_{}_retrain_{}'.format(MODEL_NAME, i))
    ...
    filtered_data = read_data('filtered_data_{}'.format(MODEL_NAME))
    gt = ddc_gt.ddc_gt(target_names=target_names)
    ...
    binmols_2, descrs_2 = get_mols_and_descr("datasets/CHEMBL25_FILTERED_2.h5")
    binmols_list =  [binmols_2, filtered_binmols[10000*i : 10000*(i+1)]]
    descrs_list = [descrs_2, filtered_descrs[10000*i : 10000*(i+1)]]
    binmols = np.concatenate(binmols_list)
    descrs = np.vstack(descrs_list)

    # 重训
    filename = 'models/model_1114_retrain_0' if i == 0 else "models/model_{}_retrain_{}".format(MODEL_NAME, i)
    ...
    int_arg = int(arg)
    print("Calculating descrs for round {}.{}".format(i, int_arg))
    ...
    properties = get_descriptors(mols, sub_mol, 'gasa/{}_{}_{}'.format(MODEL_NAME, i, int_arg))
    ...
    print('Identified {} mols.'.format(len(mols_list)))
