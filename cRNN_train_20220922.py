...
class mol_generator:
    ...
    def sample(self, sample_times: int = 4, conditions: list = []):
        ...
        # 打印结果
        self.data = pd.DataFrame(self.sani_properties, columns=self.target_names, copy=True)
        print("Generated mols:{:}, sanitized mols:{:}, validity:{:}".format(len(self.mols), len(self.sani_mols), len(self.sani_mols)/len(self.mols)))
    ...
    def filter_data(self):
        ...
        filter_list = [i >= 0.8 for i in properties_data['sub_similarity']]
...
# 0921生成的分子
generator = mol_generator(seed_smile=seed_smiles, sub_smile=sub_smiles, target_names=target_names)

binmols_1_list = []
descrs_1_list = []
for i in range(4):
    generator.load_data("datasets/generator_0921_m_{}.pickle".format(i), "datasets/generator_0921_p_{}.pickle".format(i))
    new_binmols, new_descrs = generator.filter_data()
    new_binmols = new_binmols['binmols']
    new_descrs = np.asarray([new_descrs[name] for name in target_names]).T
    
    binmols_1_list.append(new_binmols[:5000])
    descrs_1_list.append(new_descrs[:5000])

binmols_1 = np.concatenate(binmols_1_list)
descrs_1 = np.concatenate(descrs_1_list)
# 筛选的相似度0.3-1的分子
...
# 合并
binmols_list = [binmols_2, binmols_1]
descrs_list = [descrs_2, descrs_1]
...
for i in range(4, 8):
    ...
    #重新训练
    trainer = model_trainer(sub_smile=sub_smiles, model="models/model_0921", binmols=binmols, descrs=descrs)
    trainer.init_model(dataset_info)
    trainer.fit()
    trainer.save_model("models/model_0921_{}".format(i))

    #生成分子
    generator = mol_generator(seed_smile=seed_smiles, sub_smile=sub_smiles, model=trainer.model, target_names=target_names)
    target_gasas = 0.9
    generator.sample(250, [None]*7 + [target_gasas])
    generator.dump("datasets/generator_0921_m_{}.pickle".format(i), generator.sani_mols)
    generator.dump("datasets/generator_0921_p_{}.pickle".format(i), generator.sani_properties)

    #更新训练集
    filtered_binmols_data, filtered_properties_data = generator.filter_data()
