...
retrain_times = trainset_count // 2560
...
for i in range(retrain_times):
    ...
    #更新训练集
    binmols = np.asarray(filtered_binmols[2560*i : 2560*(i+1)])
    descrs = np.asarray(filtered_descrs[2560*i : 2560*(i+1)])

    #重训
    filename = 'models/model_1108_retrain_0' if i == 0 else "models/model_1112_retrain_{}".format(i)
    gt.retrain(filename, x=descrs, y=binmols, epochs=100, lr=pow(10,-4.5), lr_init=pow(10,-4.5))
    gt.save_model("models/model_1112_retrain_{}".format(i+1))
    ...
    gt.plot("results/model_1112_retrain_{}.png".format(i), title="model_1112_retrain_{}".format(i))
...
report_data.to_csv('results/report_1112.csv')
