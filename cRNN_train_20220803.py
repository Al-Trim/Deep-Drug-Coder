    ...
    for idx, binmol in enumerate(binmols_list):
        ...
        if mol:
            ...
            try:
                ...
                descriptors.append([logp, tpsa, sim, qed, hba, hbd, active, 10])
...
# Set substruct
sub_mol = Chem.MolFromSmiles("c1(C=O)cc(OC)c(O)cc1")
...
model.fit(epochs              = 300,         # number of epochs
          ...
          model_name          = "model_0803", # base name to append the checkpoints with
          checkpoint_dir      = "checkpoints_0803/",          # save checkpoints in the notebook's directory
