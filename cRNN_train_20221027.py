...
for i in range(4):
    ...
    gt.fit(model_name='model_1027_{}'.format(i), lr=pow(10,-4.5), lr_init=pow(10,-4.5))
    gt.save_model("models/model_1027_{}".format(i))
    ...
    mols, total_count, val_count, dup_count, val = gt.generate(sample_times=250, batch_input_length=256)
    properties = get_descriptors(mols, sub_mol)
    gt.set_properties(properties)
    gt.dump("datasets/generator_1027_{}.pickle".format(i), gt.sani_properties)
    sani_properties_list.append(gt.sani_properties.copy())
    gt.plot("results/model_1027_{}.png".format(i), title="model_1027_{}".format(i))
...
gt.init_model(x=descrs, y=binmols, dataset_info=dataset_info)
gt.fit(model_name='model_1027_retrain_0', lr=pow(10,-4.5), lr_init=pow(10,-4.5))
gt.save_model("models/model_1027_retrain_0")

for i in range(4):
    print("Running retrain round {}.".format(i))

    #更新训练集
    sani_properties = sani_properties_list[i]
    ...
    #重训
    gt.retrain("models/model_1027_retrain_{}".format(i), x=descrs, y=binmols, lr=pow(10,-4.5), lr_init=pow(10,-4.5))
    gt.save_model("models/model_1027_retrain_{}".format(i+1))

    #生成分子
    gt.conditions = conditions
    mols, total_count, val_count, dup_count, val = gt.generate(batch_input_length=128)
    properties = get_descriptors(mols, sub_mol)
    gt.set_properties(properties)
    gt.plot("results/model_1027_retrain_{}.png".format(i), title="model_1027_retrain_{}".format(i))
...
report_data.to_csv('results/report_1027.csv')
