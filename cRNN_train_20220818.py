...
for i in range(3):
    ...
    #训练
    trainer_1 = model_trainer(sub_smile=sub_smiles, model="model_1", binmols=binmols)
    trainer_1.init_model(dataset_info)
    trainer_1.fit()
    trainer_1.save_model("models/model_0818_{}".format(i))

    #生成
    generator_1 = mol_generator(seed_smile=seed_smiles, sub_smile=sub_smiles, model=trainer_1.model, target_names=target_names)
    generator_1.sample(10000)
    generator_1.dump("datasets/generator_0818_{}.pickle".format(i))
    ...
    rnd_idx = random.randint(0, 1335000)
    with h5py.File(dataset_filename, "r") as f:
        binmols_list.append(np.asarray(f["mols"][rnd_idx:rnd_idx+5000]))

    dataset_filename = r"datasets/data_0818_{}.h5".format(i)
    generator_1.filter_data(dataset_filename)
    with h5py.File(dataset_filename, "r") as f:
        binmols_list.append(np.asarray(f["mols"]))

# 模拟model_0813
trainer_2 = model_trainer(is_retrain=True, sub_smile=sub_smiles, model=trainer_1.model, binmols=binmols)
trainer_2.fit()
trainer_2.save_model("models/model_0818_final")
trainer_2.dump("models/trainer_2.pickle")
