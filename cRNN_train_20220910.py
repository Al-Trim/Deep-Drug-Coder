#换用GASA，尝试使用GC回收内存，采用新的相似度计算策略，训练2次+重训1次
import gc, os, sys
import numpy as np
import random
...
for i in range(2):
    ...
    #重新训练
    if i==1:
        trainer = model_trainer(sub_smile=sub_smiles, model="model_0910", binmols=binmols, descrs=descrs)
        trainer.init_model(dataset_info)
        trainer.fit()
        trainer.save_model("models/model_0910_{}".format(i))
    else:
        trainer = model_trainer(is_retrain=True, sub_smile=sub_smiles, model="models/model_0904_0", binmols=binmols, descrs=descrs)
    ...
    gc.collect()
    generator.dump("datasets/generator_0910_m_{}.pickle".format(i), generator.sani_mols)
    generator.dump("datasets/generator_0910_p_{}.pickle".format(i), generator.sani_properties)
    ...
    dataset_filename = r"datasets/data_0910_{}.h5".format(i)
...
# 重训
gc.collect()
trainer = model_trainer(is_retrain=True, sub_smile=sub_smiles, model="models/model_0910_1", binmols=binmols, descrs=descrs)
trainer.fit()
trainer.save_model("models/model_0910_final")

# 生成
generator = mol_generator(seed_smile=seed_smiles, sub_smile=sub_smiles, model=trainer.model, target_names=target_names)
generator.sample(1000, [None]*6+[1])
generator.dump("datasets/generator_0910_m_final.pickle", generator.sani_mols)
generator.dump("datasets/generator_0910_p_final.pickle", generator.sani_properties)
