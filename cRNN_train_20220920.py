...
class mol_generator:
    ...
    def filter_data(self):
        ...
        filter_list = [i >= 0.8 for i in properties_data['sim_similarity']]
...
# 训练集
generator = mol_generator(seed_smile=seed_smiles, sub_smile=sub_smiles, model='models/model_0921_0', target_names=target_names)
generator.load_data("datasets/generator_0921_m_0.pickle", "datasets/generator_0921_p_0.pickle")
...
for i in range(6):
    ...
    #设置训练集
    binmols = filtered_binmols[i:i+2000]
    descrs = filtered_descrs[i:i+2000]
    ...
    trainer.fit()
    trainer.save_model("models/model_0919_retrain_{}".format(i))

    #测试生成效果
    generator.sample(10, [None]*7+[0.7])
    d1, d2 = generator.filter_data()
